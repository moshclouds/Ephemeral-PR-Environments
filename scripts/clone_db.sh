...
case "$SERVICE_NAME" in
  order-service)
    ...
    $COMPOSE_CMD exec -T postgres-db psql -U user -d postgres -c "CREATE DATABASE ${TARGET_DB};"
    echo "Copying staging data to $TARGET_DB..."
    $COMPOSE_CMD exec -T postgres-db pg_dump -U user order_db | $COMPOSE_CMD exec -i postgres-db psql -U user -d ${TARGET_DB}
    echo "Successfully cloned Postgres DB to ${TARGET_DB}"
    ;;
  inventory-service)
    TARGET_DB="inventory_db_pr_${PR_NUMBER}"
    echo "Creating MySQL DB: $TARGET_DB"
    $COMPOSE_CMD exec -T mysql-db mysql -uroot -ppassword -e "DROP DATABASE IF EXISTS ${TARGET_DB}; CREATE DATABASE ${TARGET_DB};"
    echo "Copying staging data to $TARGET_DB..."
    $COMPOSE_CMD exec -T mysql-db mysqldump -uroot -ppassword inventory_db | $COMPOSE_CMD exec -i mysql-db mysql -uroot -ppassword $TARGET_DB
